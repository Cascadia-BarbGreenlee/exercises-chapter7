...
public class ContactMapActivity extends FragmentActivity {
		...
		if (contacts.size()>0) {
		    ...
		    for (int i=0; i<contacts.size(); i++) {
		        ...
		        try {
		        	...
		        	List<Address> geoResults = geo.getFromLocationName("<address goes here>", 1);
		            while (geoResults.size()==0) {
		                geoResults = geo.getFromLocationName("<address goes here>", 1);
		            }
		        ...
		        googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.marker)).position(point).title(currentContact.getContactName()).snippet(address));
    ...
    public void onPause() {
    	...
    	private void initDetailsButton(){
    		final Button detailButton = (Button)findViewById(R.id.buttonDetails);
    		detailButton.setOnClickListener(new View.OnClickListener() {
    							
				@Override
				public void onClick(View v) {
					// TODO Auto-generated method stub
					Intent intent = new Intent(ContactMapActivity.this, SensorActivity.class);
